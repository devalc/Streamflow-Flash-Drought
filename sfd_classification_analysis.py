...
# Categorize Mean_SFD_Flow_Percentile into 3 classes based on distribution
def categorize_flow_percentile(value, q33, q67):
    if value <= q33:
        return 'Low'
    elif value <= q67:
        return 'Medium'
...
q33 = df[target_var].quantile(0.33)
q67 = df[target_var].quantile(0.67)
...
df['Flow_Category'] = df[target_var].apply(lambda x: categorize_flow_percentile(x, q33, q67))
...
def build_classification_model(df, target_col, feature_cols, test_size=0.2):
    ...
    return rf_model, feature_importance, X_test, y_test, y_pred

def analyze_by_ecoregion(df, target_col, feature_cols):
    """Analyze feature importance by ecoregion"""
    
    print(f"\n{'='*60}")
    print("ECOREGION-SPECIFIC ANALYSIS")
    print(f"{'='*60}")
    ...
    for ecoregion in df['AGGECOREGION'].unique():
        ...
        if class_dist.min() < 5:
            print(f"  Skipping: some classes have < 5 samples")
            continue
...
def create_visualizations(df, target_col, feature_importance, ecoregion_results):
    ...
    ax4.axvline(df['Mean_SFD_Flow_Percentile'].quantile(0.33), color='red', linestyle='--', label='33rd percentile')
    ax4.axvline(df['Mean_SFD_Flow_Percentile'].quantile(0.67), color='red', linestyle='--', label='67th percentile')
    ax4.set_xlabel('Mean SFD Flow Percentile')
    ax4.set_ylabel('Frequency')
    ax4.set_title('Distribution of Target Variable')
    ax4.legend()
    
    # Plot 5: Boxplot by ecoregion
    ax5 = plt.subplot(3, 3, 5)
    df.boxplot(column='Mean_SFD_Flow_Percentile', by='AGGECOREGION', ax=ax5)
    ax5.set_xlabel('Ecoregion')
    ax5.set_ylabel('Mean SFD Flow Percentile')
    ax5.set_title('Flow Percentile by Ecoregion')
    plt.xticks(rotation=45)
    plt.suptitle('')  # Remove default title
    
    # Plot 6: Feature importance heatmap by ecoregion
    if ecoregion_results:
        ...
        if importance_matrix:
            sns.heatmap(importance_matrix, 
                       ...
                       annot=True, fmt='.3f', cmap='viridis', ax=ax6)
            ax6.set_title('Feature Importance by Ecoregion')
            plt.xticks(rotation=45, ha='right')
    
    # Plot 7: Correlation with top features
    ax7 = plt.subplot(3, 2, 5)
    top_10_features = feature_importance.head(10)['feature'].tolist()
    corr_data = df[top_10_features + ['Mean_SFD_Flow_Percentile']].corr()
    
    mask = np.triu(np.ones_like(corr_data, dtype=bool))
    sns.heatmap(corr_data, mask=mask, annot=True, fmt='.2f', 
               cmap='coolwarm', center=0, ax=ax7)
    ax7.set_title('Correlation: Top 10 Features')
    ...
    class_means.T.plot(kind='bar', ax=ax8)
    ...
    plt.tight_layout()
    plt.savefig('sfd_classification_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()

# Run the analysis
if __name__ == "__main__":
    
    # Build overall classification model
    model, feature_importance, X_test, y_test, y_pred = build_classification_model(
        df, 'Flow_Category', numeric_cols
    )
    
    # Analyze by ecoregion
    ecoregion_results = analyze_by_ecoregion(df, 'Flow_Category', numeric_cols)
    
    # Create visualizations
    create_visualizations(df, 'Flow_Category', feature_importance, ecoregion_results)    
...
# Print confusion matrix
    ...
    print(f"Class boundaries: Low ≤ {q33:.2f}, Medium {q33:.2f}-{q67:.2f}, High > {q67:.2f}")
    ...
    # Save results
    print(f"\nSaving results...")
    feature_importance.to_csv('feature_importance_classification.csv', index=False)
    
    # Save ecoregion results
    with open('ecoregion_classification_results.txt', 'w') as f:
        f.write("ECOREGION-SPECIFIC FEATURE IMPORTANCE\n")
        f.write("="*50 + "\n\n")
    ...
    print("Files saved:")
    print("  - sfd_classification_analysis.png")
    print("  - confusion_matrix.png") 
    print("  - feature_importance_classification.csv")
    print("  - ecoregion_classification_results.txt")